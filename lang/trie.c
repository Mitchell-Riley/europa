...
trie_t* trie_new(dispatch_queue_t queue)
{
	...
	if(trie)
	{
		...
		else
		{
			...
			char str[36] = {0};
			snprintf(str, sizeof(str) - 1, "europa.trie.new.%p", trie);
			trie->queue = dispatch_queue_create(str, NULL);
		}
		trie->count = 1;
...
static void trie_free(trie_t* trie)
{
	recursive_remove(trie, trie->head);
	dispatch_release(trie->queue);
	free(trie);
	trie = NULL;
}

trie_t* trie_retain(trie_t* trie)
{
	dispatch_queue_t queue = dispatch_queue_create("europa.trie.retain", NULL);
	dispatch_sync(queue, ^{ trie->count++; });
	dispatch_release(queue);
}

void trie_release(trie_t* trie)
{
	dispatch_queue_t queue = dispatch_queue_create("europa.trie.release", NULL);
	dispatch_sync(queue, ^{
		if(trie->count--)
		{
			dispatch_queue_t low = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0);
			dispatch_async(low, ^{ trie_free(trie); });
		}
	});
	dispatch_release(queue);
}
...
{
	__block trie_node_t* t;

	if(trie == NULL)
		return NULL;

	dispatch_sync(trie->queue, ^{
		trie_node_t* p;
		trie_node_t* x;
		int i;
...
static int prv_trie_delete(trie_t* trie, const char* key)
{
	trie_node_t* p;
	trie_node_t* t;
	trie_node_t* x;
